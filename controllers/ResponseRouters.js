...
exports.fetchAvg = async (req, res, next) => {
  ...
  const filter = { test_id: mongoose.Types.ObjectId(testId) };
  let cursor = Response.aggregate([{ $match: filter },
      { $group: {_id:testId,count:{$sum:"$score"}, 
    question:{ $push:{question_id:"$question_id" }  }
 }},
